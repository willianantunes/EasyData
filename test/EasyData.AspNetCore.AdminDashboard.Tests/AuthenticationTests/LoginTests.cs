...
{
    public class LoginTests : IClassFixture<AuthEnabledFixture>
    {
        private readonly HttpClient _client;

        public LoginTests(AuthEnabledFixture fixture)
        {
        ...
        public async Task PostLogin_WithNextUrl_RedirectsToNextAsync()
        {
            ...
            var formContent = new FormUrlEncodedContent(new[]
            {
                new KeyValuePair<string, string>("username", "admin"),
                new KeyValuePair<string, string>("password", "admin"),
                new KeyValuePair<string, string>("next", "/admin/Category/"),
            });
            ...
            // Assert
            Assert.Equal(HttpStatusCode.Redirect, response.StatusCode);
            Assert.Equal("/admin/Category/", response.Headers.Location.ToString());
        }
