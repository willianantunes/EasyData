...
{
    public delegate NDjangoAdminManager NDjangoAdminManagerResolver(IServiceProvider services, NDjangoAdminOptions options);

    public class NDjangoAdminOptions
    {
        public string Endpoint { get; set; } = "/api/ndjango-admin";

        public NDjangoAdminManagerResolver ManagerResolver { get; private set; }
        ...
        public void UseManager(NDjangoAdminManagerResolver managerResolver)
        {
            ManagerResolver = managerResolver;
        }

        /// <summary>
        /// Defines the function which creates and returns an instance of NDjango.Admin manager. 
        /// The manager defines all basic operations with the models and queries: creating, loading, saving and query building itself.
        /// </summary>
        /// <typeparam name="TEDManager"></typeparam>
        public void UseManager<TEDManager>() where TEDManager : NDjangoAdminManager
        {
            UseManager((services, options) => (TEDManager)Activator.CreateInstance(typeof(TEDManager), services, options));
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="NDjangoAdminOptions"/> class.
        /// </summary>
        public NDjangoAdminOptions()
        {
